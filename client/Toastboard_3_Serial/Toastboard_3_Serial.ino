...
int buttonPin = 32;      // scan button pin
...
int float_2_Pin = 3;
int float_3_Pin = 4;
int float_out_Pin = 14;

//VARIABLE INITS
int buttonState = 1;
int push_buttonState = 1;
...
int float_pins[] = {float_1_Pin, float_2_Pin, float_3_Pin};
...
pinMode(float_1_Pin, OUTPUT);
pinMode(float_2_Pin, OUTPUT);
pinMode(float_3_Pin, OUTPUT);
pinMode(float_out_Pin, OUTPUT);
...
digitalWrite(float_1_Pin,LOW);
digitalWrite(float_2_Pin,LOW);
digitalWrite(float_3_Pin,LOW);
...
//  digitalWrite(lp_ledPin,HIGH);
  ...
  if (incomingByte == sendDataOnce){
    ...
    scanchain(control_pins,I_control_pins,adc_pins,adc_Results,sampdelay, num_scans,avg_Results);      
  ...
  for (int i = 0; i < 4; i++){
    ...
    for (int x = 0; x < num_muxs; x++){
      ...
      for (int y = 0; y < num_adcs; y++){
          ...
          adc_Results[row_index][col_index] = adc_value;     
          avg_Results[row_index] = adc_value;
          }
...
float floatcheck(int control_pin_list[4], int mux_pin_list[4], int adc_pin_list[3], int float_results[48], float resholder[48], float std_dev[48], float avg_Results[48]){
  ...
  for (int j = 0; j<48; j++){
    ...
    if (j < 16){
     ...
     if (resholder[j] > 80 && resholder[j] < 100){
      float_results[j] = 1;
     } 
    }
     else{
     if (resholder[j] > 75 && resholder[j] < 120){
      float_results[j] = 1;
     } 
     }
...
float newfloatcheck(int control_pin_list[4], int mux_pin_list[4], int adc_pin_list[3], int float_results[48], float resholder[48], float std_dev[48], float avg_Results[48], int float_pin_list[3]){
 ...
 digitalWrite(float_out_Pin,HIGH);
  ...
  for (int j = 0; j<48; j++){
    ...
    if (avg_Results[j] < 0.15){
    
    int control_pin = control_pin_list[j % 4];
    int I_control_pin = mux_pin_list[(j/4) % 4];
    int adc_pin = adc_pin_list[j/16];
    int float_pin = float_pin_list[j/16]; 
  
    digitalWrite(I_control_pin,HIGH);
    digitalWrite(control_pin, HIGH);
    digitalWrite(float_pin, HIGH);
    
    delayMicroseconds(floatdelay);
    
    resholder[j] = analogRead(adc_pin);
    
    if (resholder[j] > 700 && resholder[j] < 1300){
      float_results[j] = 1;
    }
    else {
    float_results[j] = 0;
...
float floatcheckDebug(int control_pin_list[4], int mux_pin_list[4], int adc_pin_list[3], int float_results[48], float resholder[48], float std_dev[48], float avg_Results[48]){
  ...
  for (int k=0;k<48;k++){
   ...
   Serial.print(k);
   Serial.print(": ");
   Serial.print(foobar[k][0]);
   Serial.print(" ");
   Serial.print(foobar[k][99]);
   float slope = (foobar[k][0] - foobar[k][99]) / 100.0;
   Serial.print(" ");
...
//------------------------------------------------------------------------------------------------------------
String formatJsonData(float avg_results[48], int float_results[48], unsigned long starttime) {
  String rows;
  ...
  for (int i=0;i<24;i++) {
    if (float_results[i] == 0) {
      ...
      rows += buffer; //avg_results[i]; //buffer;
    } else {
      rows += "\"f\"";
    }
    if (i != 23) {
